...
public sealed record CanDeviceSnapshot(
    string VertexId, bool IsActive, string ListenerState, string ReceptionState,
    DateTimeOffset? LastReceivedAtUtc, long ReceivedPackets, string? LastError);

public sealed class CanDeviceStatus(string vertexId, bool isActive)
{
    private readonly object _sync = new();
    private string _state = isActive ? "starting" : "inactive";
    ...
    public CanDeviceSnapshot Snapshot(DateTimeOffset now, TimeSpan staleAfter)
    {
        lock (_sync)
        {
            var reception = !isActive ? "inactive"
                : _lastReceived is null ? "waiting"
                : now - _lastReceived.Value >= staleAfter ? "stale" : "recent";
            return new(vertexId, isActive, _state, reception, _lastReceived, _receivedPackets, _lastError);
        }
    }
}
